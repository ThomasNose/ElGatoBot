...
from gaming.currency import balance_both, combat_pay, user_balance, bet, bet_payout
from gaming.monsters import stat_upgrade, monster_combat
from gaming.racing.race import race_status, move_horse
from gaming.racing.event import event
...
class horse_race(discord.ui.View):
    ...
    async def update_message(self, content = None, winner = None):
        await self.message.edit(embed=self.create_embed(content, winner),view = self)


    def create_embed(self, content = None, winner = None):
        embed = discord.Embed(title="Horse race")
    ...
    def update_buttons(self):
        ...
        self.bet4.disabled = True
        self.bet5.disabled = True
        self.bet6.disabled = True
    

    @discord.ui.button(label="Horse 1: 5.00", style = discord.ButtonStyle.primary)
    async def bet1(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 1}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)

    @discord.ui.button(label="Horse 2: 5.00", style = discord.ButtonStyle.primary)
    async def bet2(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 2}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)

    @discord.ui.button(label="Horse 3: 5.00", style = discord.ButtonStyle.primary)
    async def bet3(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 3}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)

    @discord.ui.button(label="Horse 4: 5.00", style = discord.ButtonStyle.primary)
    async def bet4(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 4}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)

    @discord.ui.button(label="Horse 5: 5.00", style = discord.ButtonStyle.primary)
    async def bet5(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 5}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)

    @discord.ui.button(label="Horse 6: 5.00", style = discord.ButtonStyle.primary)
    async def bet6(self, interaction:discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        payload = '{"type": "horse", "mode": "standard", "horse": 6}'
        deposit = await bet(interaction.user.id, interaction.guild.id, 5.00, self.message.id, payload)
    ...
    async def start_race(self):
        ...
        start = [space.replace("\u2003","") for space in self.tracks]
        await self.interaction.response.send_message(embed=self.create_embed(content=start), view=self)
        ...
        self.tracks = [move_horse(self, num, self.track, start_time) for num, self.track in enumerate(self.tracks)]
        track_1, track_2, track_3, track_4, track_5, track_6 = self.tracks
        self.race = race_status(self.track_top, self.track_bottom, self.track_pos, *self.tracks)
        await self.update_message()
        ...
        # Updating the event
        for key, value in self.track_pos.items():
            if "1st" in str(value):
                self.winner = key

        event_payload = {"winner": self.winner, "places": self.track_pos}
        await event(self.message.id, self.interaction.guild.id, payload = event_payload)


        # Paying out winners
        winner_list = await bet_payout(self.interaction.guild.id, self.message.id)
        await asyncio.sleep(10)
        await self.update_message(winner_list, True)
        #try:
        #    await self.message.edit(content=self.race)
        #except Exception as e:
        #    print(f"Error sending/updating the message: {e}")